...
const CharacterSelection = () => {
  ...
  const handleSlotClick = (slotId: number) => {
    if (characters[slotId]) {
      ...
      console.log(
        'Setting active character from slot:',
        slotId,
        characters[slotId]
      );
  ...
  const handleCreateNewCharacter = () => {
    ...
    const emptySlot = Array.from({ length: 6 }, (_, index) => index + 1).find(
      slotId => !characters[slotId]
    );
          ...
          <div className={styles.activeCharacterContent}>
            <div className={styles.activeCharacterBasicInfo}>
              ...
              <div className={styles.activeCharacterInfo}>
                ...
                <p className={styles.activeCharacterClass}>
                  {activeCharacter.characterClassId.charAt(0).toUpperCase() +
                    activeCharacter.characterClassId.slice(1)}
                </p>
                <p className={styles.activeCharacterLevel}>
              ...
              <div className={styles.activeExperienceBar}>
                <div className={styles.activeExperienceProgress}>
                  <div
                    ...
                    style={{
                      width: `${(activeCharacter.experience / activeCharacter.maxExperience) * 100}%`,
                    }}
                  ></div>
                </div>
                <span className={styles.activeExperienceText}>
                  {activeCharacter.experience} / {activeCharacter.maxExperience}{' '}
                  XP
                </span>
              </div>
            </div>
            ...
            <div className={styles.activeCharacterMeta}>
              <div className={styles.activeMetaItem}>
                <span>Created:</span>
                <span>
                  {new Date(activeCharacter.createdAt).toLocaleDateString(
                    'de-DE'
                  )}
                </span>
              </div>
      ...
      <div className={styles.characterGrid}>
        <div className={styles.header}>
          <div className={styles.headerContent}>
            ...
            <p>
              Choose your character or create a new one to start your adventure
            </p>
          </div>
        </div>
