...
import { Award, Star, Target, Shield, Sword, Activity, Crown, Zap, Heart, Trophy } from 'lucide-react';
import styles from '../Character.module.scss';

const BadgesTab: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'combat' | 'skill' | 'achievement'>('all');
          ...
          {filteredBadges.map((badge) => (
            ...
            >
              <div className={styles.badgeIcon}>
                <badge.icon 
                  size={24} 
                  style={{ color: badge.owned ? getRarityColor(badge.rarity) : '#6b7280' }}
                />
                {badge.owned && (
          ...
          {ownedBadges.slice(0, 3).map((badge) => (
            <div key={badge.id} className={styles.recentBadgeItem}>
              <div className={styles.recentBadgeIcon}>
                <badge.icon size={20} style={{ color: getRarityColor(badge.rarity) }} />
              </div>
