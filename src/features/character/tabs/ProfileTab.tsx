...
import { useCharacter, useCharacterClasses, useStatAllocation } from '../../../core/contexts/GameContext';
...
const ProfileTab: React.FC = () => {
  ...
  const skills = [
    ...
    { id: 'enchanting', name: 'Enchanting', icon: Snowflake, level: 1, color: '#87CEEB' },
    { id: 'breeding', name: 'Breeding', icon: Egg, level: 1, color: '#FFD700' },
    { id: 'combat', name: 'Combat', icon: Sword, level: 1, color: '#DC143C' },
    { id: 'exploration', name: 'Exploration', icon: Mountain, level: 1, color: '#8B0000' },
    { id: 'taming', name: 'Taming', icon: Baby, level: 1, color: '#20B2AA' },
    { id: 'defense', name: 'Defense', icon: ShieldIcon, level: 1, color: '#4682B4' },
    { id: 'jewelry', name: 'Jewelry', icon: Crystal, level: 1, color: '#DDA0DD' },
    { id: 'herbalism', name: 'Herbalism', icon: Flower, level: 1, color: '#90EE90' },
    { id: 'archery', name: 'Archery', icon: Target, level: 1, color: '#DEB887' },
  ...
  const getTotalPendingPoints = () => {
    return Object.values(pendingChanges).reduce((sum, change) => sum + change, 0);
  };
  ...
  const handleStatChange = (statKey: keyof typeof character.stats, amount: number) => {
    ...
    <div className={styles.tabContent}>
      ...
      <div className={`${styles.section} ${styles.characterOverview}`}>
        ...
        <div className={styles.characterHeader}>
          ...
          <div className={styles.characterInfo}>
            ...
            <p><strong>Klasse:</strong> {classInfo?.name || character.characterClass}</p>
      ...
      <div className={styles.attributesAndInfoContainer}>
          ...
          <div className={styles.statValueContainer}>
            <span className={styles.statValue}>
              ...
              {pendingChanges.strength && pendingChanges.strength !== 0 && (
                <span className={`${styles.pendingChange} ${pendingChanges.strength > 0 ? styles.positive : styles.negative}`}>
                  ({pendingChanges.strength > 0 ? '+' : ''}{pendingChanges.strength})
                </span>
              )}
            </span>
            {character.availableStatPoints > 0 && (
              <div className={styles.statControls}>
                <button
                  ...
                  disabled={!pendingChanges.strength || pendingChanges.strength <= 0}
                  title="Verringern"
                >
                  <Minus size={14} />
                </button>
          ...
          <div className={styles.statValueContainer}>
            <span className={styles.statValue}>
              ...
              {pendingChanges.agility && pendingChanges.agility !== 0 && (
                <span className={`${styles.pendingChange} ${pendingChanges.agility > 0 ? styles.positive : styles.negative}`}>
                  ({pendingChanges.agility > 0 ? '+' : ''}{pendingChanges.agility})
                </span>
              )}
            </span>
            {character.availableStatPoints > 0 && (
              <div className={styles.statControls}>
                <button
                  ...
                  disabled={!pendingChanges.agility || pendingChanges.agility <= 0}
                  title="Verringern"
                >
                  <Minus size={14} />
                </button>
        ...
        <div className={styles.statItem}>
          ...
          <div className={styles.statValueContainer}>
            <span className={styles.statValue}>
              ...
              {pendingChanges.intelligence && pendingChanges.intelligence !== 0 && (
                <span className={`${styles.pendingChange} ${pendingChanges.intelligence > 0 ? styles.positive : styles.negative}`}>
                  ({pendingChanges.intelligence > 0 ? '+' : ''}{pendingChanges.intelligence})
                </span>
              )}
            </span>
            {character.availableStatPoints > 0 && (
              <div className={styles.statControls}>
                <button
                  ...
                  disabled={!pendingChanges.intelligence || pendingChanges.intelligence <= 0}
                  title="Verringern"
                >
                  <Minus size={14} />
                </button>
          ...
          <div className={styles.statValueContainer}>
            <span className={styles.statValue}>
              ...
              {pendingChanges.vitality && pendingChanges.vitality !== 0 && (
                <span className={`${styles.pendingChange} ${pendingChanges.vitality > 0 ? styles.positive : styles.negative}`}>
                  ({pendingChanges.vitality > 0 ? '+' : ''}{pendingChanges.vitality})
                </span>
              )}
            </span>
            {character.availableStatPoints > 0 && (
              <div className={styles.statControls}>
                <button
                  ...
                  disabled={!pendingChanges.vitality || pendingChanges.vitality <= 0}
                  title="Verringern"
                >
                  <Minus size={14} />
                </button>
          ...
          <div className={styles.statValueContainer}>
            <span className={styles.statValue}>
              ...
              {pendingChanges.luck && pendingChanges.luck !== 0 && (
                <span className={`${styles.pendingChange} ${pendingChanges.luck > 0 ? styles.positive : styles.negative}`}>
                  ({pendingChanges.luck > 0 ? '+' : ''}{pendingChanges.luck})
                </span>
              )}
            </span>
            {character.availableStatPoints > 0 && (
        ...
        <div className={styles.infoItem}>
          ...
          <span>{character.createdAt !== "Unbekannt" ? formatDate(character.createdAt) : character.createdAt}</span>
        </div>
        <div className={styles.infoItem}>
          <span>Letzter Login:</span>
          <span>{character.lastLogin !== "Unbekannt" ? formatDate(character.lastLogin) : character.lastLogin}</span>
        </div>
        </div>
      </div>
