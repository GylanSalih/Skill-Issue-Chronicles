...
import {
  ...
  WoodcuttingSession,
  WoodcuttingResult,
} from '../services/woodManager';
import { WoodTypeConfig, getAllWoodTypes } from '../services/woodConfig';
...
export const useWoodcutting = (): UseWoodcuttingReturn => {
  ...
  const [activeSession, setActiveSession] = useState<WoodcuttingSession | null>(
    null
  );
  const [currentWoodAmounts, setCurrentWoodAmounts] = useState<
    ...
    getAllWoodTypes().forEach(woodType => {
      amounts[woodType.id] =
        gameState.resources.secondary[
          woodType.id as keyof typeof gameState.resources.secondary
        ] || 0;
    });
  ...
  // Starte Woodcutting
  const startChopping = useCallback(
    (woodTypeId: string, loop: boolean = false): boolean => {
      const success = woodManager.startWoodcutting(
        woodTypeId,
        playerLevel,
        loop
      );
  ...
  const canChopWood = useCallback(
    (woodTypeId: string): boolean => {
  ...
  const getWoodAmount = useCallback(
    (woodTypeId: string): number => {
