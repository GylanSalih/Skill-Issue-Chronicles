...
export const useBossCombat = (player: Player, boss: Boss) => {
  ...
  const calculateDamage = useCallback(
    (attacker: { attack: number }, defender: { defense: number }): number => {
  ...
  const playerAttack = useCallback(() => {
    ...
    setCombatState(prev => ({
      ...
      combatLog: [
        ...prev.combatLog,
        `${player.name} attacks ${boss.name} for ${damage} damage!`,
      ],
    ...
    if (newBossHealth > 0) {
      setTimeout(() => {
        ...
        const newPlayerHealth = Math.max(
          0,
          combatState.playerHealth - bossDamage
        );
        ...
        setCombatState(prev => ({
          ...
          combatLog: [
            ...prev.combatLog,
            `${boss.name} attacks ${player.name} for ${bossDamage} damage!`,
          ],
  ...
  const playerDefend = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setCombatState(prev => ({
        ...
        combatLog: [
          ...prev.combatLog,
          `${boss.name} attacks ${player.name} for ${bossDamage} damage (defended)!`,
        ],
        isDefeat: newPlayerHealth <= 0,
      }));
    }, 1000);
  ...
  const playerMagic = useCallback(() => {
    ...
    setCombatState(prev => ({
      ...
      combatLog: [
        ...prev.combatLog,
        `${player.name} casts a magic spell for ${magicDamage} damage!`,
      ],
      isVictory: newBossHealth <= 0,
    }));
  ...
  const playerSpecial = useCallback(() => {
    ...
    setCombatState(prev => ({
      ...
      combatLog: [
        ...prev.combatLog,
        `${player.name} uses a special ability for ${specialDamage} damage!`,
      ],
      isVictory: newBossHealth <= 0,
    }));
