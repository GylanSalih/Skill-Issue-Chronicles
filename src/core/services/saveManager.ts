...
export class SaveManager {
  ...
  static exportToFile(
    ...
    link.download =
      filename ||
      `idle-game-save-${new Date().toISOString().split('T')[0]}.json`;
      ...
      reader.onload = event => {
        try {
          ...
          if (this.validateSaveData(saveData)) {
            ...
            if (
              saveData.characters &&
              Object.keys(saveData.characters).length > 0
            ) {
              localStorage.setItem(
                'idleGameCharacters',
                JSON.stringify(saveData.characters)
              );
              console.log(
                'Imported characters saved to localStorage:',
                saveData.characters
              );
            }
